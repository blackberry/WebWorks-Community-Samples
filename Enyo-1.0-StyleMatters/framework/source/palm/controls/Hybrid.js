...
enyo.kind({
	...
	published: {
		...
		/** if true, plugin will register for touch events from WebKit */
		passTouchEvents: false,
		/** background color to use for displaying plugin before initial draw or during resize.  It's a string of the form,
		    "rrggbb" using hex digits, with "000000" being black, "FFFFFF" being white. If null, use default gray color.
	...
	create: function() {
		...
		if (window.PalmSystem) {
			...
			if (this.bgcolor) {
				this.setAttribute("bgcolor", this.bgcolor);
			}
	...
	nullTouchHandler: function() { },

	rendered: function() {
		this.inherited(arguments);
		this.pluginReady = false; // when re-rendered, we need to wait for ready again
		if (this.hasNode()) {
			/* register for touch events with a null handler. This is only way to get our
			 * our system WebKit to pass these in to the plugin */
	...
	destroy: function() {
		if (this.passTouchEvents && this.node) {
			this.node.removeEventListener("touchstart", this.nullTouchHandler);
			this.node.removeEventListener("touchend", this.nullTouchHandler);
			this.node.removeEventListener("touchmove", this.nullTouchHandler);
		}
		this.inherited(arguments);
	},
